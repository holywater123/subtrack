...
-- Lets a debt/credit-card payment be paid FROM a chosen wallet, so the
-- money actually leaving that account shows up in its balance. Tracks
-- only the principal (kept separate from expenses so it isn't
-- double-counted as spend) - the interest portion still goes through
-- expenses.debt_id above.
